...
df = pd.read_csv('../imigue_data/labels_20200831.csv')
...
# for reshuffled simulations, run: 
#df['class'] = df['class'].sample(frac=1).reset_index(drop=True)

#load input npy (gesture keypoint)
gesture_inputs= np.load("npy/inputs_no_padding_noConfidence.npy", allow_pickle = True)
...
def train(model, lr, num_epochs, train_dl, test_dl, sim_count,epoch_performance):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        np.save("npy/lstm_array/simulation/lstm/lstm_epoch"+str(epoch),cpu_lstm)
        np.save("npy/lstm_array/simulation/ys/"+str(a)+"&"+str(b)+"_ys_epoch"+str(epoch),cpu_ys)
        cpu_lstm= np.load("npy/lstm_array/simulation/lstm/lstm_epoch"+str(epoch)+".npy", allow_pickle = True)
        cpu_ys= np.load("npy/lstm_array/simulation/ys/"+str(a)+"&"+str(b)+"_ys_epoch"+str(epoch)+".npy", allow_pickle = True)
    ...
    for (a, b) in sim:
        ...
        for i in range(len(df['class_selected'])):
            if df['class_selected'][i] ==1:
                new_inputs.append(gesture_inputs[i])
        ...
        gesture= np.load("npy/simulation/pad_gesture_input/pad__pose_class"+str(a)+"&class"+str(b)+"_simulation.npy", allow_pickle = True)
        print('geature_input shape', gesture.shape)
        print(len(sample_valid))
        run_model(gesture, sample_valid)
